...
/*   Updated: 2023/03/25 19:17:14 by jalvarad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_coord	get_normal(t_program *p, t_coord *point, t_coord rd)
{
	...
	aux1.x += 0.001;
	aux2.x -= 0.001;
	norm.x = min_sdf(aux1, p, rd) - min_sdf(aux2, p, rd);
	aux1 = *point;
	aux2 = *point;
	aux1.y += 0.001;
	aux2.y -= 0.001;
	norm.y = min_sdf(aux1, p, rd) - min_sdf(aux2, p, rd);
	aux1 = *point;
	aux2 = *point;
	aux1.z += 0.001;
	aux2.z -= 0.001;
	norm.z = min_sdf(aux1, p, rd) - min_sdf(aux2, p, rd);
	return (v_unit(norm));
}
